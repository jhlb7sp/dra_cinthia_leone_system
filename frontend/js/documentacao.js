function gerarPedidoDocumentacao() {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();
    const paciente = document.getElementById('paciente').value;
    const radio = document.querySelector('input[name="simpcomp"]:checked');
    const simplecompleta = radio ? radio.value : '';
    ...
    for (let via = 1; via < 2; via++) {
        ...
        doc.setFont(undefined, 'bold');
        doc.text("CNPJ:", 125, inicioY + 15);
        doc.setFont(undefined, 'normal');
        doc.text("60.997.717/0001-81", 140, inicioY + 15);
        ...
        doc.setFontSize(14);
        doc.setFont(undefined, 'normal');
        doc.text(`Documentação ortodôntica ${simplecompleta}`, 20, inicioY + 67);
    ...
    doc.save(`SolicitaçãoDoc_${paciente}.pdf`);
}