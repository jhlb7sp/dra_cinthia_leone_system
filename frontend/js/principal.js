...
const iframePrincipal =
    document.getElementById('iframePrincipal') ||
    document.querySelector(
        'iframe[name="iframePrincipal"]'
    );


const lembreteManutencoes =
    document.getElementById('lembreteManutencoes');


const textoLembreteManutencoes =
    document.getElementById('textoLembreteManutencoes');
...
/* =========================================
   FECHA MENU AO ABRIR UMA TELA
========================================= */

document
    .querySelectorAll(
        '.menu-link[target="iframePrincipal"], .submenu-link[target="iframePrincipal"]'
    )
    .forEach(
        link => {
...
function exibirMensagemMotivacional() {
    ...
    if (
        ...
        frase =
            'Chega por hoje, bora pra casa descansar!';

    }


    mensagemEl.textContent =
        `${saudacao}, Dra. Cinthia: ${frase}`;
...
if (
    ...
    lembreteManutencoes.addEventListener(
        'click',
        () => {
            ...
            if (
                ...
                iframePrincipal.src =
                    'retorno-pacientes.html?filtro=atencao';
...
if (
    ...
    iframePrincipal.addEventListener(
        'load',
        () => {
...
async function verificarLembretesManutencao() {
    ...
    try {
        ...
        const resposta =
            await fetch(
                'http://localhost:3000/api/manutencoes/lembretes'
            );
...
async function verificarAniversariantes() {
    ...
    try {
        ...
        const resposta =
            await fetch(
                'http://localhost:3000/api/pacientes/aniversariantes-hoje'
            );
...
function criarPopupAniversario(
    paciente,
    index
) {
    ...
    popup.innerHTML =
        `
            <span class="nome-aniversariante">
                ${primeiroNome}
            </span>

            <span class="emoji-aniversario">
                🥳
            </span>
        `;
    ...
    popup.addEventListener(
        ...
        () => {
            ...
            if (
                ...
                alert(
                    'Esse paciente não possui telefone cadastrado.'
                );
...
function mostrarTooltipMenu(
    elemento
) {

    /*
       Tooltip aparece somente quando
       o menu estiver recolhido.
    */
    ...
    /*
       Coloca à direita do menu.
    */

    tooltipMenu.style.left =
        `${rect.right + 10}px`;
    ...
    */
    ...
    const tooltipRect =
        tooltipMenu.getBoundingClientRect();
