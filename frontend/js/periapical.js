...
function gerarPeriapical() {
    ...
    for (let via = 1; via < 2; via++) {
        ...
        doc.setFontSize(14);
        doc.setFont(undefined, 'normal');
        doc.text("Radiografia periapical do(s) dente(s):", 20, inicioY + 67);

        // Lista de medicamentos
        doc.setFontSize(14);
        doc.setFont(undefined, 'normal');
        doc.text("Para fins de diagnóstico.", 20, inicioY + 125);
    ...
    doc.save(`Solicitação_Periapical_${paciente}.pdf`);
}